...
uploadanthros <- function(finaltable) {
  ...
  # new data or not
  print(paste("Type 'yes' to input data into the anthropometrics table in the database"))
  r <- readline(prompt="Enter here: ")
  if (r=="yes") {
    print(paste("Please specify the range of dates that you would like to have added to the anthropometrics table"))
    ...
    subset <- finaltable[finaltable$DATE>=first & finaltable$DATE<=last,]
  ...
  print("Your table in MySQL has been updated")
}
